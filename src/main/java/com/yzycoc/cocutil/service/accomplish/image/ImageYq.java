...
public class ImageYq {
    ...
    public ClanResult get() {
        ...
        CocApiAndCqCustom.saveTxt(YqHtmlCustom.htmlAll + byId.getHtml());
        HttpYqUtils httpUtils = HttpYqUtils.getInstance();
        httpUtils.setTimeout(20000);
        httpUtils.setWaitForBackgroundJavaScript(20000);
        try {
            String htmlPageStr = httpUtils.getHtmlPageResponse("file:///"+YqHtmlCustom.htmlAll);
            ...
            String html = htmlPageStr.replaceAll("class=\"graphs\"","style=\"font-family:Arial;\"")
                    .replaceAll("class=\"timeScaleHeader\"","style=\"font-size: 12px;padding: 0px;vertical-align: middle;font-variant: small-caps;font-weight: bold;max-width: 55px;\"")
                    .replaceAll("Loot Available", "战<br/>利<br/>品").replaceAll("Players Online", "玩<br/>家<br/>在<br/>线").replaceAll("Shielded Players", "玩<br/>家<br/>离<br/>线")
                    .replaceAll("style=\"height: 100px;\"", "style=\"height: 100px;padding-left: 21px;\"")
                    .replaceAll("style=\"height: 20px;\"", "style=\"height: 20px;padding-left: 23px;\"")
                    .replaceAll("style=\"height: 250px;padding-left:12px;\"", "style=\"height: 250px;padding-left:18px;\"")
                    .replaceAll("style=\"background-color:#EEEEEE;\"", "style=\"background-color:#EEEEEE;FONT-FAMILY: '隶书';FONT-SIZE: 20px;\"")
                    .replaceAll("Time", "")
                    .replaceAll("<div class=\"rotate\">\r\n" +
                            "                                        Loot Index\r\n" +
                            "                                      </div>","")
                    .replaceAll("<div class=\"rotate\">Loot Index</div>","")
                    ;
            long endTime = System.currentTimeMillis(); // 获取结束时间
            System.out.println("代码渲染耗时： " + (endTime - startTime) + "ms");
            byId.setHtml(html);
            //生成图片
            getImage(byId,uid);
            long datadifference = TimeUtiles.datadifference(byId.getHtmltime());
            datadifference = 600 -datadifference;
            if(datadifference<=60) {
                datadifference = 60;
            }
            System.out.println("鱼情图片生成完毕，耗时： " + (System.currentTimeMillis() - startTime) + "ms");
            return new ClanResult(true,uid, ConfigParameter.filePath_Yq,"jpg");
        } catch (Exception e) {
            e.printStackTrace();
            return new ClanResult(false,"鱼情图片合成失败；请稍后重试！");
        }
    }


    public void getImage(YuQing yQ, String uid) {
        Graphics2D g = null;
        try {
            ...
            if(yQ.getHtml()!=null) {
                Html2Image html2Image = Html2Image.fromHtml(yQ.getHtml());
				...
				/*Thumbnails.of(cocImageAll).outputFormat("jpg").scale(1f).outputQuality(0.9f).toFile(
						new File("G:\\酷Q\\酷Q Air\\新建文件夹\\"+TimeUtils.getStringDate("MM年dd月 HH点mm")));*/
            }else {
                Thumbnails.of(Image).outputFormat("jpg").scale(1f).outputQuality(0.5f).toFile(
                        new File(ConfigParameter.filePath_Yq+"\\"+uid));
            }
            Image.flush();
        } catch (Exception e) {
