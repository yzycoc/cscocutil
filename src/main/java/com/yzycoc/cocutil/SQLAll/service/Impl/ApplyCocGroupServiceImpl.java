...
import com.yzycoc.cocutil.service.accomplish.image.ImagePlayer;
...
public class ApplyCocGroupServiceImpl  extends ServiceImpl<ApplyCocGroupMapper, ApplyCocGroup> implements ApplyCocGroupService {
    ...
    public Result addUser(AddUserForm addUser) {
        ...
        try {
            ...
            if(applyGroup == null){
                applyGroup = this.query().eq("group_number", addUser.getGroupNumber()).eq("type", typeMsg).one();
                if (applyGroup == null) {
    ...
    public Result updateUser(UpdateUserForm updateUser) {
        ...
        try {
            ...
            if(applyGroup == null){
                applyGroup = this.query().eq("group_number", updateUser.getGroupNumber()).eq("type", updateUser.getMsg()).one();
                if (applyGroup == null) {
