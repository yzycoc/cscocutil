...
public class MyDemo<T> {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < troops.size(); i++) {
            ...
            if(village.equals("home")){
                ...
                /*System.out.println("case \""+JSON.getString("name")+"\":\n" +
                        "                            data.set"+setname+"(JSONString(troopsJSON,\"level\"));\n" +
                        "if(JSONString(troopsJSON,\"level\").equals(JSONString(troopsJSON,\"maxLevel\")) && !\"-\".equals(troopsJSON.getString(\"level\"))){\n" +
                        "\t\t\t\t\t\t\t\tdata.setIsMax"+name+"(true);\n" +
                        "\t\t\t\t\t\t\t}else{\n" +
                        "\t\t\t\t\t\t\t\tdata.setIsMax"+name+"(false);\n" +
                        "\t\t\t\t\t\t\t}\n" +
                        "                            break;");*/
                int length = (h.get(key)).length() ;
                length = 55 + (length * 15);
                System.out.println("{align:'center',field: '"+name+"', title: '"+h.get(key)+"',sort: true,width: "+length+",style:\"background: #FF6666;border:1px solid #e6e6e6\", templet: function(d){\n" +
                        "\t\t\t\t\t\tvar clas = \"\";if(d.isMax"+name+"){clas = \"maxLevel home\";}else{clas = \"minLevel home\";}return \"<p class='\"+clas+\"'>\"+d."+name+"+\"</p>\";\n" +
                        "\t\t\t\t\t}},");
                //System.out.println("private Boolean isMax"+name+";");
            }
        ...
        for (int i = 0; i < troops.size(); i++) {
            ...
            if(JSON.getString("village").equals("builderBase")){
                ...
                /*System.out.println("case \""+JSON.getString("name")+"\":\n" +
                        "                            data.set"+setname+"(JSONString(troopsJSON,\"level\"));\n" +
                        "if(JSONString(troopsJSON,\"level\").equals(JSONString(troopsJSON,\"maxLevel\")) && !\"-\".equals(troopsJSON.getString(\"level\"))){\n" +
                        "\t\t\t\t\t\t\t\tdata.setIsMaxbb"+name+"(true);\n" +
                        "\t\t\t\t\t\t\t}else{\n" +
                        "\t\t\t\t\t\t\t\tdata.setIsMaxbb"+name+"(false);\n" +
                        "\t\t\t\t\t\t\t}\n" +
                        "                            break;");*/
                int length = (h.get(key)).length() ;
                length = 55 + (length * 15);
                System.out.println("{align:'center',field: 'bb"+name+"', title: '"+h.get(key)+"',sort: true ,width: "+length+",style:\"background: #FFFF00;\", templet: function(d){\n" +
                        "\t\t\t\t\t\tvar clas = \"\";if(d.isMaxbb"+name+"){clas = \"maxLevel builderBase\";}else{clas = \"minLevel builderBase\";}return \"<p class='\"+clas+\"'>\"+d.bb"+name+"+\"</p>\";\n" +
                        "\t\t\t\t\t}},");
                //System.out.println("private Boolean isMaxbb"+name+";");
            }
        }
        ...
        System.out.println(""); System.out.println("");
        System.out.println("");
        troops = jsonObject.getJSONArray("spells");
        for (int i = 0; i < troops.size(); i++) {
            JSONObject JSON = troops.getJSONObject(i);
            String village = JSON.getString("village");
            if(village.equals("home")){
               ...
               /* System.out.println("case \""++"\"");
                ...
                /*System.out.println("case \""+JSON.getString("name")+"\":\n" +
                       "                            data.set"+setname+"(JSONString(troopsJSON,\"level\"));\n" +
                        ...
                        "\t\t\t\t\t\t\t}\n" +
                        "                            break;");*/
                int length = (h.get(key) + "法术").length() ;
                length = 55 + (length * 15);
                System.out.println("{align:'center',field: '"+name+"', title: '"+h.get(key)+"法术',sort: true,width:"+length+",style:\"background: #3399CC;\", templet: function(d){\n" +
                        "\t\t\t\t\t\tvar clas = \"\";if(d.isMax"+name+"){clas = \"maxLevel spells\";}else{clas = \"minLevel spells\";}return \"<p  class='\"+clas+\"'>\"+d."+name+"+\"</p>\";\n" +
                        "\t\t\t\t\t}},");
                //System.out.println("private Boolean isMax"+name+";");
            }
